...
#         print(attn.sum(dim=-1), attn.shape)
    ...
    def __init__(self, num_flops, last_chn):
        ...
        self.conv_list = nn.ModuleList([
            nn.Sequential(
                ...
                nn.Conv2d(self.num_channels[ii+1], self.num_channels[ii+2], 3, 1, 1),
                nn.BatchNorm2d(self.num_channels[ii+2]),
                nn.ReLU(inplace=True)
                # nn.ReLU(inplace=False)
            ) for ii in range(0, len(self.num_channels)-1, 2)
        ])
...
#         self.batch_attn = []
        ...
        # patch_embed
        if cnn_stem:
            self.patch_embed1 = ShallowCNNstem('1GF', last_chn=embed_dims[0])
        else:
...
#         print(self.patch_embed1)
        ...
        if return_attn:
            ret_attn = [[False]*(j-1)+[True] for j in depths]
        else:
            ret_attn = [[False]*j for j in depths]
...
#             if i > 0])
        ...
        # self.classify_global_pool = nn.AdaptiveAvgPool2d((1,1))

#         classification head
        # self.head = nn.Linear(embed_dims[3], num_classes) if num_classes > 0 else nn.Identity()

        self.out_dim = embed_dims[3]
        self.apply(self._init_weights)
...
class mit_b0(MixVisionTransformer):
    def __init__(self, return_attn=False, cnn_stem=False, **kwargs):
        super(mit_b0, self).__init__(
            patch_size=4, embed_dims=[32, 64, 160, 256], num_heads=[1, 2, 5, 8], mlp_ratios=[4, 4, 4, 4],
            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 2, 2], sr_ratios=[8, 4, 2, 1],
            drop_rate=0.0, drop_path_rate=0.1, nf=256, return_attn=return_attn, cnn_stem=cnn_stem)
...
class mit_b1(MixVisionTransformer):
    def __init__(self, return_attn=False, cnn_stem=False, **kwargs):
        super(mit_b1, self).__init__(
            ...
            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 2, 2], sr_ratios=[8, 4, 2, 1],
            drop_rate=0.0, drop_path_rate=0.1, nf=512, return_attn=return_attn, cnn_stem=cnn_stem)
...
class SegFormer_Body(nn.Module):
    def __init__(
        ...
        self.mit = mit_func(return_attn=return_attn)
...
if __name__ == '__main__':
    id_gpu = 0
    model = mit_b4(cnn_stem=True)
#     model = nn.Sequential(
#                 SegFormer_Body(mit_b0),
#                 SegFormer_Head
#     .to(torch.device(f'cuda:{id_gpu}'))
...
#     in_size = (3, 640, 640)
    ...
    flops, params = get_model_complexity_info(model, in_size, as_strings=True, print_per_layer_stat=True)
    print('Flops:  ' + flops)
    print('Params: ' + params)
